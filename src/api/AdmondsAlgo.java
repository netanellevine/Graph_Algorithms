...
public class AdmondsAlgo {
        ...
        while (!q.isEmpty()) {
            ...
            for (int kni : cur.getNeighbours()) {
                ...
                if (!visited.contains(kni) && ni_mate != -1) {
                    ...
                    tree.connect(e.getSrc(),e.getDest(),e.getWeight());
                    tree.addNode(g.getNode(ni_mate));
                    EdgeData con = g.getEdge(kni,ni_mate);
                    tree.connect(con.getSrc(),con.getDest(),con.getWeight());
    ...
    public List<Integer> backTracking(int src, int dest, DirectedWeightedGraph tree) {
        ...
        path.add(curr);
        while (curr != dest) {
            var nei = tree.getNode(curr).getNeighbours();
            if (nei.size() == 1) {
                // only one way to go
